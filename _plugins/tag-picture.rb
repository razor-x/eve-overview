    ...
    def render(context)
      ...
      if settings['markup'] == 'picturefill'
        ...
        source_keys.reverse.each { |source|
          ...
          source_tags += "<span data-src=\"#{baseurl}#{instance[source][:generated_src]}\"#{media}></span>\n"
        }
        ...
        picture_tag = "<span #{html_attr_string}>\n"\
                      "#{source_tags}"\
                      "<noscript>\n"\
                      "<img src=\"#{baseurl}#{instance['source_default'][:generated_src]}\" alt=\"#{html_attr['data-alt']}\" />\n"\
                      "</noscript>\n"\
                      "</span>\n"
        ...
        source_keys.each { |source|
          if source == 'source_default'
            source_tags += "<img src=\"#{baseurl}#{instance[source][:generated_src]}\" alt=\"#{html_attr['alt']}\" />\n"
          else
            source_tags += "<source src=\"#{baseurl}#{instance[source][:generated_src]}\" media=\"#{instance[source]['media']}\" />\n"
          end
        }
        ...
        source_keys.reverse.each do |source|
          interchange_data << "[#{baseurl}#{instance[source][:generated_src]}, #{source == 'source_default' ? '(default)' : instance[source]['media']}]"
        end

        picture_tag = %Q{<img src="#{baseurl}#{instance['source_default'][:generated_src]}" data-interchange="#{interchange_data.join ', '}" #{html_attr_string} />}
      end
